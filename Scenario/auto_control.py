"""
    Now starting auto driving simulation...
    Use key tab to switch camera from FPS to TPS.
    Press key pad num 7,8,9 to take action to the anomaly signal lights.
    Press key pad num 1,2,3 to ignore the anomaly signal lights.
    press esc to escape the simulation.
"""
...
class World(object):
    ...
    def restart(self):
        ...
        # Get a fixed blueprint for player (small car).
        blueprint = get_actor_blueprints(self.world, self._actor_filter, self._actor_generation)[6]
...
# KeyboardControl handles the keyboard events
    ...
    def parse_events(self, client, world, clock, sync_mode):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                f = open(logfile,'a')
                f.write("The user interrupt the simulation.")
                f.close()
                return True
...
# Dashboard for information of Server and anomaly signals
class HUD(object):
    ...
    def render(self, display):
        if self._show_info:
            ...
            # anomaly ui
            # ------
            anomaly_surface = pygame.Surface((360, 220))
            anomaly_surface.set_alpha(100)
            display.blit(anomaly_surface,(920, 500))
...
# FadingText for information hint
...
# HelpText to handle text output using pygame
...
# CollisionSensor in vehicles to detect collision
...
# LaneInvasionSensor in vehicle to detect if lane switching is allowed
...
# GnssSensor in vehicle is a kind of radar for scanning surroundings
...
# IMUSensor in vehicle is a kind of sensor to detect the speed, accelaration...and such physical parameters
...
# RadarSensor in vehicles to get distance of other entities. 
...
# CameraManager decides how cameras are installed in vehicles (switch view)
...
# game_loop() handles refresh ticking from carla server and update data every tick
# anomaly is inserted in this area to perfrom anytime generation
...
def game_loop(args):
    ...
    try:
        ...
        # countdown timer insert

        counter,text = float(args.t),args.t
        font = pygame.font.SysFont('Consolas', 15)

        # setup a list of signal switches
        ...
        # load anomaly icons in UI
        icon_door = pygame.image.load("icons\\door.png")
        icon_seatbelt = pygame.image.load("icons\\seatbelt.png")
        icon_coolant = pygame.image.load("icons\\coolant.png")
        icon_engine = pygame.image.load("icons\\engine.png")
        icon_fuel = pygame.image.load("icons\\fuel.png")
        ...
        if args.s == 'scenario_1':
            ...
            while True:
                ...
                if counter <= 0:
                    sys.exit()
                # after every tick Ui should update information 
...
# main() for receiving parameters from generator.py
def main():
